# Cheeko Platform — Complete System Overview (+ dev box)

> Cross-repo reference for the Cheeko kids' voice-AI toy platform. Compiled 2026-07-09 from code
> exploration of all five components. **All paths and ports in this copy are the DigitalOcean dev
> box `root@64.227.170.31`** — read live from the server on 2026-08-12. Local checkouts are not
> referenced; see [section 6](#6-dev-box-digitalocean-6422717031) for host, routes and deploys.

## Components at a glance

| Component | pm2 process | Path on dev box | Stack | Port(s) |
|---|---|---|---|---|
| Manager API | `manager-api` | `/root/xiaozhi-esp32-server/main/manager-api-node` | Node/Express 4 + Prisma 7 (Supabase Postgres `tsiocygczplmnjpqmutc`) | 8002 (`/toy` context path) |
| MQTT Gateway | `mqtt-gateway` | `/root/xiaozhi-esp32-server/main/mqtt-gateway` | Node (CommonJS), EMQX, LiveKit SDK | UDP 8884 (audio), 127.0.0.1:8091 (internal HTTP), 8004 (health) |
| LiveKit Voice Worker | `picoclaw-livekit` | `/root/picoclaw` (binary `build/picoclaw-livekit`) | Go, LiveKit agent worker, in-process picoclaw agent | 8193 (health) |
| Line-Art / AI-Imagine server | `line-art` | `/root/line_art` (venv `env/`) | Python FastAPI + FLUX image gen | 8010 (WS `/ws`) |
| Manager Web (admin UI) | `manager-web` | `/root/xiaozhi-esp32-server/main/manager-web` | Node/npm | 8001 |
| Admin dashboard (persona editor, LiveKit Test tab) | `admin-dashboard` | `/root/xiaozhi-esp32-server/main/admin-dashboard` | Node `server.js` | 4000 (public via `dash.64-227-170-31.sslip.io`) |
| Device simulator | — | `client.py` (run from a workstation against the box) | Python (paho-mqtt, pyaudio, opuslib) | — |

Infra the dev box runs itself in Docker: **LiveKit server** (7880-7881/tcp, 7882-7900/udp — self-hosted,
not LiveKit Cloud), **EMQX** (1883, 8883, 8083-8084, dashboard 18083), Redis (6380→6379), MySQL (3306, legacy).
Caddy terminates TLS on 80/443 for `otadev.cheekoai.in` and the two `*.sslip.io` hosts.

Other pieces referenced: Cerebrium media API (music/story bots), Qdrant (RFID RAG),
Mem0 (memories), AWS S3 + CloudFront (`cdn.cheekoai.in` / `dsmzc13oafp54.cloudfront.net`), Firebase (parent app auth).

Full dev-box detail — routes, deploy recipe, migration gotchas — in [section 6](#6-dev-box-digitalocean-6422717031).
...
- ⚠️ `/root/xiaozhi-esp32-server/main/manager-api-node/.env` holds live DB/AWS/SMTP secrets.
...
## 3. LiveKit Voice Worker (`picoclaw`, `/root/picoclaw`)
...
- **Key env** (`/root/picoclaw/.env`, config `/root/.picoclaw/config.json`): `livekit_service.*` in config.json (server_url, api_key/secret), `PICOCLAW_LIVEKIT_MANAGER_API_URL` = `http://127.0.0.1:8002/toy` (and `MANAGER_API_URL` = `http://64.227.170.31:8002/toy`), `PICOCLAW_LIVEKIT_MANAGER_API_SERVICE_KEY`, `PICOCLAW_VAD_THRESHOLD`, `PICOCLAW_VAD_ENDPOINT_MS`.
...
- **Run**: pm2 `line-art` at `/root/line_art` (venv `env/bin/python`), listening on **8010**; the gateway reaches it at `LINE_ART_WS_URL=ws://127.0.0.1:8010/ws`. Optional `WS_SHARED_SECRET` auth on hello.
...
## 6. Dev box (DigitalOcean, 64.227.170.31)

The only environment an agent may deploy to. **Never** production (`ota.cheekoai.in` → 139.59.7.72)
or the EKS cluster — those deploys are Rahul's.

`ssh root@64.227.170.31`. Everything runs as root under pm2, fronted by Caddy (not nginx).

### Public entry points (`/etc/caddy/Caddyfile`)

| Host | Route | Backend |
|---|---|---|
| `otadev.cheekoai.in` | `/toy/v1/*` | :8000 (legacy WS) |
| | `/toy/ota/*` | :8002 manager-api |
| | `/mcp/*` | :8003 |
| | everything else | :8002 manager-api |
| `dash.64-227-170-31.sslip.io` | all | :4000 admin dashboard (persona editor + LiveKit Test tab) |
| `lk.64-227-170-31.sslip.io` | all | :7880 LiveKit signalling |

sslip.io is used so Caddy can issue real certs without a DNS record; the dashboard needs HTTPS
because its Test tab calls `getUserMedia`, and the LiveKit host needs TLS too or the browser
rejects `ws://` as mixed content.

### pm2 processes (`pm2 list`)

| id | name | cwd | entry |
|---|---|---|---|
| 1 | `manager-api` | `/root/xiaozhi-esp32-server/main/manager-api-node` | npm (:8002) |
| 2 | `manager-web` | `/root/xiaozhi-esp32-server/main/manager-web` | npm (:8001) |
| 3 | `mqtt-gateway` | `/root/xiaozhi-esp32-server/main/mqtt-gateway` | `app.js` (:8004 health, 127.0.0.1:8091 internal, UDP 8884) |
| 24 | `line-art` | `/root/line_art` | `env/bin/python` (:8010) |
| 25 | `admin-dashboard` | `/root/xiaozhi-esp32-server/main/admin-dashboard` | `server.js` (:4000) |
| 26 | `picoclaw-livekit` | `/root/picoclaw` | `build/picoclaw-livekit` (:8193 health) |
| 27 | `visitors-register` | `/root/cheeko_visitors_registere_server` | npm (:8080) — unrelated to the toy |

Note manager-api, manager-web, mqtt-gateway and admin-dashboard all live in **one** repo,
`/root/xiaozhi-esp32-server`, under `main/`.

### Infra in Docker

| Container | Image | Ports |
|---|---|---|
| `livekit-server` | `livekit/livekit-server` | 7880-7881/tcp, 7882-7900/udp |
| `emqx` | `emqx/emqx-enterprise:5.10.0` | 1883, 8883, 8083-8084, 18083 (dashboard) |
| `redis-container` | `redis` | 6380→6379 |
| `mysql-container` | `mysql` | 3306 (legacy; manager-api uses Supabase Postgres) |

So LiveKit is **self-hosted on this box**, not LiveKit Cloud.

### Repos / branches (as of 2026-08-12)

| Path | Branch | HEAD |
|---|---|---|
| `/root/picoclaw` | `stt-ptt-batch` | `b79b8b7a` |
| `/root/xiaozhi-esp32-server` | `feat/per-age-banks` | `8c91ad23` |
| `/root/line_art` | — (not a git checkout) | |

Config for the worker: `/root/.picoclaw/config.json`, env in `/root/picoclaw/.env`.

**Database:** manager-api here points at Supabase project **`tsiocygczplmnjpqmutc`** (port 5432).
Local dev manager-api uses a *different* project (`shlrfpbqkfnxqcmuatvs`:6543) — schema and seed
changes must be applied to **both**.

### Deploying

No CI to this box; deploys are manual. Windows cross-compile of the Go worker fails (opus needs
cgo), so build on the server. Confirm local HEAD == server HEAD first, then ship only changed files:

```bash
tar -cf - <files> | ssh root@64.227.170.31 "tar -xf - -C /root/picoclaw"
```

- **Go:** `export PATH=$PATH:/usr/local/go/bin && export CGO_LDFLAGS='-lc++ -lc++abi' && make build-livekit && pm2 restart picoclaw-livekit`
- **Node:** `node --check` each file, then `pm2 restart manager-api --update-env` / `mqtt-gateway`
- **Prisma schema change → `npx prisma generate` BEFORE restarting manager-api**, else
  `select:{new_field}` throws at runtime. Prisma 7's `prisma.config.ts` does not auto-load `.env`;
  it needs `import 'dotenv/config'`.

⚠️ `server.js` runs `runPrismaMigrations()` on boot and this DB has `_prisma_migrations`, so
`pm2 restart manager-api` applies **every** unapplied migration in the tree at once — verified
2026-08-12, an expand/contract pair landed 0.26s apart with no soak between. Any migration you
don't want applied yet must not be in the tree when you restart.

Other gotchas: `go test ./pkg/livekit/` fails on the server with `libten_vad.so: cannot open
shared object file` (test-loader only — the built binary loads it fine). STT/TTS/LLM selection
comes from the manager API and overrides code defaults. `detailed_trace_enabled=true` is still on,
so kids' conversation content sits in plaintext logs — turn off before scale.
