...
| `k8s/livekit-capacity-test-deployment.yaml` | Off-by-default c6a.large canary Deployment for measuring per-pod LiveKit voice-agent capacity without sending traffic to the production `cheeko-agent` pool. |
...
2. LiveKit dispatches the `cheeko-agent` worker.
...
### Runtime settings live in `config.json`, not the Deployment env

Most `livekit_service.runtime` settings are only read from `config.json` inside the
`picoclaw-config` secret. The `env:` struct tags in `pkg/config/config.go` are
documentation; only a few keys (for example `PICOCLAW_LIVEKIT_RUNTIME_VAD_THRESHOLD`)
have a matching `os.Getenv` call in `cmd/picoclaw-livekit/main.go`. Adding an env var
to `k8s/livekit-deployment.yaml` for a key without one is silently ignored.

`greeting_mode` is the one that bites. It has no env reader:

| Value | Behaviour |
| --- | --- |
| `dynamic` (default when unset) | Greeting is generated by the LLM from the character's `greeting_prompt` and soul. |
| `fallback` | Hardcoded phrase in `pkg/livekit/audio_pipeline.go`. Ignores the persona entirely — no character voice, no child's name, no expression tags. |
...
kubectl --context $Context -n $Namespace get secret picoclaw-config `
  -o jsonpath='{.data.config\.json}' | base64 -d | Select-String greeting_mode
```

Changing it means recreating the secret (below) and rolling the pods; config is read at
startup, so a secret edit alone does not take effect.
