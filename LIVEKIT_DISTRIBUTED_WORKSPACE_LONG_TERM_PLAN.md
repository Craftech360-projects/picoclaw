...
12. Add durable workspace artifact persistence before making workspaces ephemeral.
   - Status: implemented and code-verified.
   - Purpose: files generated by LiveKit tools, such as songs, stories, notes, and other small text artifacts, must survive worker changes. The tool sandbox remains the device workspace, but the durable source of truth must be Manager API/Postgres.
   - Manager API behavior: added `device_workspace_artifacts` with unique `(mac_address, relative_path)`, service-key protected `PUT /toy/agent/device/:mac/artifacts`, `GET /toy/agent/device/:mac/artifacts`, and `GET /toy/agent/device/:mac/artifacts/content`.
   - PicoClaw behavior: after successful LiveKit `write_file`, the worker mirrors the text artifact to Manager API; on room start, it hydrates recent text artifacts back into `workspace-device-<mac>` before the agent uses workspace tools.
   - Scope guard: store small text artifacts in Postgres first. Large binary files, audio, images, and long-term object storage can be added later when needed.
   - Files: `D:\cheeko-backend\main\manager-api-node\prisma\schema.prisma`, `D:\cheeko-backend\main\manager-api-node\prisma\migrations\20260422_add_workspace_artifacts\migration.sql`, `D:\cheeko-backend\main\manager-api-node\src\services\agent.service.js`, `D:\cheeko-backend\main\manager-api-node\src\routes\agent.routes.js`, `D:\cheeko-backend\main\manager-api-node\tests\unit\agent.workspace-artifacts.test.js`, `D:\picoclaw\pkg\livekit\workspace_artifacts.go`, `D:\picoclaw\pkg\livekit\agent_bridge.go`, `D:\picoclaw\pkg\livekit\agent_bridge_artifacts_test.go`, `D:\picoclaw\cmd\picoclaw-livekit\manager_artifact_store.go`, and `D:\picoclaw\cmd\picoclaw-livekit\manager_artifact_store_test.go`.
